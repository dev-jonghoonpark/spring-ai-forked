...
package org.springframework.ai.gemini.chat.client;
...
class GeminiChatClientIT extends AbstractIT {

	private static final Logger logger = LoggerFactory.getLogger(GeminiChatClientIT.class);
	...
	void re2() {
		...
		ChatClient chatClient = ChatClient.builder(this.chatModel)
			.defaultOptions(GeminiChatOptions.builder()
				.model(GeminiApi.ChatModel.GEMINI_2_O_FLASH.getValue()).build())
	...
	void beanStreamOutputConverterRecords() {
		...
		Flux<ChatResponse> chatResponse = ChatClient.create(this.chatModel)
				.prompt()
				.options(GeminiChatOptions.builder().build())
				.advisors(new SimpleLoggerAdvisor())
				.user(u -> u
